...
def get_queue_by_lines(lines,split_str="\t",max_length=2)
...
config_file = "#{root_dir}/data/asian_id_name_mapping.tsv"
record_file = "#{root_dir}/data/face_download_records_#{$split}_#{$n}.txt"
...
if $flag == 0 || $flag == 1
  ...
  if $queue_names.empty?
    ...
    $queue_names = get_queue_by_lines(lines,"\t",2)
  end
  (1..worker_threads).each do |i|
    url_threads<<Thread.new do
      loop do
        ...
        puts "#################URLs[#{th_no}] threds[#{i}] for #{queue_name[0]}-#{queue_name[1]} start..."
        hash_imgs = []
        page_index = index = 0
        face = 1
        loop do
          if hash_imgs.length <= index            
            request_url = "#{url_api}&face=#{face}&rn=60&pn=#{60*page_index}&#{url_query_string({"word"=>queue_name[1],"step_word"=>queue_name[1]},true)}" rescue ""
          ...
          break if (queue_name.length-2)>=max_length || page_index>9999
        end
        if (queue_name||[]).length > 2
          ...
          puts "#################URLs[#{th_no}] threds[#{i}] for #{queue_name[0]}-#{queue_name[1]} end with #{queue_name.length-2}"
        end
      end
    end
  end
  ...
  (1..(worker_threads/2)).each do |i|
    download_threads<<Thread.new do
      loop do
        ...
        puts "#################Dwonloads[#{th_no}] threds[#{i}] for #{queue_url[0]}-#{queue_url[1]} start with #{total_length}"
        next if ignore_exist&&File.exist?(folder_path)&&total_length>=each_imgs
        Dir.mkdir folder_path unless File.exist?(folder_path)
        download_urls = []
        ((total_length*1.1+2).to_i..(queue_url.length-1)).each do |j|
          total_length += 1
        ...
        puts "#################Dwonloads[#{th_no}] threds[#{i}] for #{queue_url[0]}-#{queue_url[1]} end with #{total_length}"
      end
    end
  end
